...
from ._download import download_openmic2018_hierarchy


class TrainingOpenMIC2018Dataset(TrainingDataset):
    def __init__(
        self,
        ...
        openmic2018_root = os.path.join(hyperaudioset_cache_dir, "data", "OpenMIC2018")

        url = "https://github.com/tky823/hyperaudioset/releases/download/v0.0.0/openmic2018.json"

        if openmic2018_root:
            os.makedirs(openmic2018_root, exist_ok=True)

        filename = os.path.basename(url)
        path = os.path.join(openmic2018_root, filename)
        chunk_size = 8192

        if not os.path.exists(path):
            download_openmic2018_hierarchy(url, path, chunk_size=chunk_size)
...
class EvaluationOpenMIC2018Dataset(EvaluationDataset):
    def __init__(
        self,
        is_symmetric: bool = False,
    ) -> None:
