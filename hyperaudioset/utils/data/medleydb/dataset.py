...
from ._download import download_medleydb_hierarchy


class TrainingMedleyDBDataset(TrainingDataset):
    def __init__(
        self,
        ...
        url = "https://github.com/tky823/hyperaudioset/releases/download/v0.0.0/medleydb.json"

        if medleydb_root:
            os.makedirs(medleydb_root, exist_ok=True)

        filename = os.path.basename(url)
        path = os.path.join(medleydb_root, filename)
        chunk_size = 8192

        if not os.path.exists(path):
            download_medleydb_hierarchy(url, path, chunk_size=chunk_size)
...
class EvaluationMedleyDBDataset(EvaluationDataset):
    def __init__(
        self,
        is_symmetric: bool = False,
    ) -> None:
